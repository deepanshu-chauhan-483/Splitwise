...
export default function Balances() {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <header>
        <h1 className="text-2xl font-bold text-gray-800">Balances</h1>
        <p className="text-gray-600 mt-1">
          Review overall amounts and group balances. Select a group to view
          per-group balances and suggested settlements.
        </p>
      </header>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Overall balances</h2>
          <button
        ...
        <div className="mt-3">
          {loading && overallList.length === 0 ? (
            ...
            <p className="text-sm text-gray-600">No balances to show.</p>
          ) : (
  ...
  {overallList.map((b, i) => {
    ...
    return (
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Group balances & settlements</h2>
          <div className="flex items-center gap-3">
            <label htmlFor="groupSelect" className="text-sm text-gray-600 mr-2 hidden sm:inline">
              Group:
            </label>
            <select
              id="groupSelect"
        ...
        <div className="mt-4">
          {!selectedGroupId ? (
            <p className="text-sm text-gray-600">Select a group to load balances and suggestions.</p>
          ) : groupList.length === 0 ? (
            <p className="text-sm text-gray-600">No balances for this group.</p>
          ) : (
           <ul className="divide-y divide-gray-100">
  {groupList.map((b, i) => {
      ...
      <li key={i} className="flex items-center justify-between py-2">
        ...
        <div className="mt-6">
          ...
          {suggestionsList.length === 0 ? (
            <p className="text-sm text-gray-600">No suggested transfers. Click "Suggest settlements" to compute.</p>
          ) : (
            <div>
              <SettlementView
              ...
              />
              {busyRecording && <p className="text-sm text-gray-500 mt-2">Recording settlement...</p>}
            </div>
          )}
        </div>
      </section>
